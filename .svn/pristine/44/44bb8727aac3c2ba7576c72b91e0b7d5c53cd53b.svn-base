...
namespace WrongSearch_Client.UC
{
    public partial class StatementUC : UserControl
    {
      ...
      //  public int i = 4;
        ...
        // 방나가기
        //exitRoom√clientForm.user.classNumber+나가는 사람 이름+나가는 사람 방번호√
        private void btnExit_Click(object sender, EventArgs e)
        {
            NetworkStream ns = clientForm.clientSocket.GetStream();

            string msg = "exitRoom√" + clientForm.user.classNumber+"," +clientForm.user.name+ ","+clientForm.user.room[0] +"√";
            byte[] buf = Encoding.UTF8.GetBytes(msg);

            ns.Write(buf, 0, buf.Length);
            ns.Flush();

            clientForm.user.room[0] = 0;
