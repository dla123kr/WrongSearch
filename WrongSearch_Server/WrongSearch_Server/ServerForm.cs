...
namespace WrongSearch_Server
{
    public partial class ServerForm : Form
    {
        ...
        public void SendRoomCreated(string msg)
        {
            string[] msgArray = msg.Split('√');
            ...
            for (int i = 0; i < allClient.Count; i++)
            {
                if (msgArray[1] == allClient[i].user.classNumber)
                {
                    allClient[i].user.room[0] = RoomNumber; // 방번호
                    allClient[i].user.room[1] = 0; // 방장입니다.
                    allClient[i].user.room[3] = 1; // 이 방엔 1명있음.

                    NetworkStream ns = allClient[i].clientSocket.GetStream();
                    int room_num = RoomNumber;
                    string msg2 = "roomCreated√" + room_num.ToString() + "√" +
                        allClient[i].user.classNumber + "√null√";
                    byte[] buf = Encoding.UTF8.GetBytes(msg2);
                    ns.Write(buf, 0, buf.Length);
                    ns.Flush();

                    host_num = allClient[i].user.classNumber;
                }
            }
            for (int j = 0; j < allClient.Count; j++)
            {
                if (msgArray[1] != allClient[j].user.classNumber)
                {
                    NetworkStream ns = allClient[j].clientSocket.GetStream();
                    int room_num = RoomNumber;
    ...
    public class handleClient
    {
        ...
        private void RecvRoom(string msg)
        {
            ...
            {
                //serverForm.SendRoomCreated(msg);
                string host_num = null;
